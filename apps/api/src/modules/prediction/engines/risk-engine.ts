...
export class RiskEngine {
  ...
  public computeHistoricalExpectedShortfall(
    ...
    if (!returns || returns.length < 60) {
      return {
        var: null,
        cvar: null,
        sampleCount: returns ? returns.length : 0,
      };
    }
  ...
  public verifyRiskInvariants(): {
    ...
    results['CRASH_SCENARIO'] = Math.abs(crashDd - 0.5) < 1e-4;
    ...
    const zeroEs = this.computeHistoricalExpectedShortfall(
      zeroVarReturns,
      0.05,
    );
    results['ZERO_VARIANCE_RETURNS'] =
      zeroEs.var !== null &&
      zeroEs.cvar !== null &&
      Number.isFinite(zeroEs.cvar);
    ...
    results['ANALYTICAL_EXPECTED_SHORTFALL'] =
      normalizedCvar >= 1.7 && normalizedCvar <= 2.4;
  ...
  calculateRisk(
    ...
    const stopLossPrice = parseFloat(
      Math.max(0.01, price - stopMultiplier * atr).toFixed(2),
    );
    ...
    const rewardRiskRatio = parseFloat(
    ...
    if (historicalCloses && historicalCloses.length >= 61) {
      ...
      for (let i = 1; i < historicalCloses.length; i++) {
        ...
        if (prev > 0)
          historicalReturns.push((historicalCloses[i] - prev) / prev);
      }
    }
    ...
    const tailRiskPercent =
      es.cvar !== null ? parseFloat((es.cvar * 100).toFixed(2)) : null;
    ...
    const liquidityScore =
      vol > 0 ? Math.max(1.0, Math.log10(price * vol)) : 1.0;
    ...
    const normVol = Math.min(1.0, annualizedVol / 0.5);
    const normDownsideDev = Math.min(1.0, downsideDev / 0.35);
    const normDrawdown =
      maxDrawdown60d !== null ? Math.min(1.0, maxDrawdown60d / 0.25) : 0.5;
    ...
    const normIlliquidity = liquidityFlag
      ? 1.0
      : Math.max(0, (8.5 - liquidityScore) / 3.0);
    ...
    if (compositeRiskScore >= MODEL_CONFIG.RISK.STATE_THRESHOLDS.EXIT) {
      ...
      compositeRiskScore >= MODEL_CONFIG.RISK.STATE_THRESHOLDS.HIGH_RISK
    ) {
      riskState = 'HIGH_RISK';
    } else if (
      compositeRiskScore >= MODEL_CONFIG.RISK.STATE_THRESHOLDS.CAUTION
    ) {
      riskState = 'CAUTION';
    }
    ...
    const expectedHorizonReturn =
      winProb * expGainPct - downsideProbability * expLossPct;
    ...
    const horizonVariance = Math.max(
      0.0001,
      (Math.pow(annualizedVol, 2) / 252) * 5,
    );
    ...
    const kellySuggestedWeight = parseFloat(
      Math.min(
        MODEL_CONFIG.PORTFOLIO.MAX_SINGLE_STOCK_WEIGHT,
        Math.max(0, optimalWeight),
      ).toFixed(4),
    );
    ...
    if (riskState === 'EXIT' || riskState === 'EMERGENCY')
    ...
    return {
      ...
      maxDrawdown60d:
        maxDrawdown60d !== null ? parseFloat(maxDrawdown60d.toFixed(4)) : null,
