...
export class DecisionEngine {
  ...
  makeDecision(
    ...
    if (dataQuality === 'LOW' || risk.liquidityFlag || signalQuality === 'LOW' || risk.downsideProbability === null) {
      return 'NO_TRADE';
    }
    ...
    if (downsideProb >= strongSellDownside || prob20d <= MODEL_CONFIG.DECISION.PROBABILITY_THRESHOLDS.STRONG_SELL) {
      return 'STRONG_SELL';
    }

    if (downsideProb >= sellDownside || prob20d <= MODEL_CONFIG.DECISION.PROBABILITY_THRESHOLDS.SELL) {
      return 'SELL';
    }

    if (downsideProb >= MODEL_CONFIG.DECISION.DOWNSIDE_THRESHOLDS.REDUCE || prob20d <= MODEL_CONFIG.DECISION.PROBABILITY_THRESHOLDS.REDUCE) {
      return 'REDUCE';
    }
    ...
    if (prob20d >= strongBuyProbHurdle && rrRatio >= MODEL_CONFIG.DECISION.REWARD_RISK_THRESHOLDS.STRONG_BUY) {
    ...
    if (prob20d >= buyProbHurdle && rrRatio >= MODEL_CONFIG.DECISION.REWARD_RISK_THRESHOLDS.BUY) {
      if (expectedValue !== null && expectedValue <= 0) {
        return 'HOLD';
      }
      return 'BUY';
